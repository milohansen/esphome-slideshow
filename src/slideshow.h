...
class OnAdvanceTrigger : public Trigger<size_t> {
 public:
  explicit OnAdvanceTrigger(SlideshowComponent *parent) {
    parent->add_on_advance_callback([this](size_t index) { this->trigger(index); });
  }
};
...
class OnQueueUpdatedTrigger : public Trigger<size_t> {
 public:
  explicit OnQueueUpdatedTrigger(SlideshowComponent *parent) {
    parent->add_on_queue_updated_callback([this](size_t size) { this->trigger(size); });
  }
};
...
class OnErrorTrigger : public Trigger<std::string> {
 public:
  explicit OnErrorTrigger(SlideshowComponent *parent) {
    parent->add_on_error_callback([this](std::string error) { this->trigger(error); });
  }
};

// Actions
template<typename... Ts> class AdvanceAction : public Action<Ts...> {
 public:
...
template<typename... Ts> class PreviousAction : public Action<Ts...> {
 public:
...
template<typename... Ts> class RefreshQueueAction : public Action<Ts...> {
 public:
